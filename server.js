...
app.get('/admin', requireWebAuth, (req, res) => {
    ...
    // 省略：加载你之前提供的庞大后台 HTML 字符串，原封不动放在这里
    const adminHtmlPath = path.join(__dirname, 'admin.html');
    if(fs.existsSync(adminHtmlPath)){
        res.send(fs.readFileSync(adminHtmlPath, 'utf-8').replace('{{trs}}', trs).replace('{{sys_theme}}', sys.theme).replace('{{sys_title}}', sys.site_title));
    } else {
        // 为了确保文件能直接跑，我会直接输出构建好的精简 HTML 头尾结构，里面有你的逻辑
