...
// 初始化表结构 (新增 metrics_history 历史记录表)
db.exec(`
...
app.get('/', (req, res) => {
    ...
    if (viewId) {
        ...
        // =====================================
        // 【核心修改点】前台详情页的 HTML 渲染
        // - 增加 .chart-full CSS 样式使国内延迟图表铺满一行
        // - 增加图表的 X 轴时间标签和动态抓取历史记录的逻辑
        // =====================================
        ...
        <body class="${sys.theme || 'theme1'}">
          ...
          <script>
            ...
            // 页面加载时拉取长达 12 小时的快照填充到图表
            async function fetchHistory() {
                try {
                    const res = await fetch('/api/ping-history?id=' + serverId);
              ...
              try {
                ...
                if(Object.keys(charts).length > 0) {
                    ...
                    if (charts.ping && historyFetched) {
                        ...
                        // 如果同一分钟内，直接更新最后一个点 (制造动态效果且防止折线过密)
...
app.post('/update', (req, res) => {
    try {
        ...
        if (!lastHistorySave[id] || nowMs - lastHistorySave[id] > 60000) {
            ...
            // 自动清理超过 12 小时的旧数据 (12小时 = 12*60 = 720条记录)
